...
class Request:
    ...
    def get_next_process_time(self, tp=True):
        if tp:
            if self.cur_token==-1:
                return self.input_len*workloads_dict[self.workload_type].info_args["tp_t_in"]
            else:
                return workloads_dict[self.workload_type].info_args["tp_t_out"]
        else:
            if self.cur_token==-1:
...
def generate_workloads():
    info1 = {"qps": args.qps1, "tp_t_in": TIME_UNIT*0.66, "st_len_in": 30, "tp_t_out": TIME_UNIT*0.66, "st_len_out": 5, "t_in": TIME_UNIT, "t_out":TIME_UNIT}
    info0 = {"qps": args.qps0, "tp_t_in": TIME_UNIT*0.66, "st_len_in": 1, "tp_t_out": TIME_UNIT*2*0.66, "st_len_out": 100, "t_in": TIME_UNIT, "t_out":TIME_UNIT*2}
...
def print_metrics(metrics, max_kv):
    ...
    with open(args.output_filename+".csv", "a") as csvfile:
        ...
        write_content = [args.policy, args.output_var, args.rate_scale, max_kv]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", choices=["fcfs", "interleave", "sjmlfq", "sjmlfqmp", "emlfq"])
